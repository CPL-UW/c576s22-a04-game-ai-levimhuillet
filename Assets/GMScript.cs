...
namespace AI {
    ...
    public class GMScript : MonoBehaviour {
        ...
        const int AI_DEPTH = 4; // AI: how many turns ahead AI will look to determine optimal drop
        ...
        private int EvaluateEnemyPieceScore(Vector3Int[] piece, Vector3Int[] chunk, int depth, int targetCol, bool drop = false) {
            if (depth == 1) {
                // base case
                ...
                return 100 * (int)(BOUNDS_MAX - combined.Average(p => p.y))  // HIGHEST SCORE = LOWEST AVERAGE 
                    - (int)(200 * piece.Average(p => Mathf.Abs(targetCol - p.x))); // AI: Decrease scores for locations farther away from shortest col
            }
            else {
                // AI: recursively generate another layer of checks
                var enemyGoLeft = ShiftPiece(piece, -1, -1, false);
                var enemyGoRight = ShiftPiece(piece, 1, -1, false);
                var enemyGoRotate = ShiftPiece(RotatePiece(piece, false), 1, -1, false);
                var enemyStay = ShiftPiece(piece, 0, -1, false);
                ...
                if (!validOptions.Any()) return EvaluateEnemyPieceScore(piece, chunk, 1, targetCol);
                var maxScore = validOptions.Max(p => EvaluateEnemyPieceScore(p, _enemyChunk, depth - 1, targetCol));
                validOptions = validOptions.Where(p => EvaluateEnemyPieceScore(p, _enemyChunk, depth - 1, targetCol) == maxScore).ToArray();
                ...
                for (int i = 1; i < validOptions.Length; ++i) {
                    ...
                    if (EvaluateEnemyPieceScore(option, chunk, 1, targetCol) > EvaluateEnemyPieceScore(validOptions.ElementAt(maxOptionIndex), chunk, 1, targetCol)) {
                        maxOptionIndex = i;
                    }
                }

                return EvaluateEnemyPieceScore(validOptions.ElementAt(maxOptionIndex), chunk, 1, targetCol) - 1; // -1 for each move away it would require
            }
        }

        private Vector3Int[] EnemyChooseAction(Vector3Int[] piece, int depth) {
            if (null == piece) return null;
            ...
            var maxScore = validOptions.Max(p => EvaluateEnemyPieceScore(p, _enemyChunk, depth, _enemyTargetCol, true));
            validOptions = validOptions.Where(p => EvaluateEnemyPieceScore(p, _enemyChunk, depth, _enemyTargetCol, true) == maxScore).ToArray();
            if (DEBUG_MODE) Debug.Log($"max score = {maxScore}; options = {validOptions.Length}");
            return validOptions.ElementAt(Random.Range(0, validOptions.Count()));
        }

        private int FindShortestCol(Vector3Int[] chunk) {
            // AI: bias the piece's movement towards a short column
            if (chunk == null) {
                return Random.Range(_minEx, _maxEx);
            }

            int shortestCol = _minEx - 1;
            int shortestHeight = BOUNDS_MAX + 1;

            List<int> options = new List<int>();

            for (int col = _minEx; col <= _maxEx; ++col) {
                int currHeight = 0;
                ...
                for (var row = -BOUNDS_MAX; row <= BOUNDS_MAX; row++) // just MIN_BOUND to MAX_BOUND?
                {
                    foreach (var p in chunk) {
                        if (p.y == row && p.x == col) {
                            currHeight = BOUNDS_MAX + p.y;
                            //Debug.Log("enemy piece at " + p.x + ", " + (BOUNDS_MAX + p.y));
                        }
                    }
                }
                ...
                if (currHeight < shortestHeight) {
                    ...
                    options.Clear();
                    options.Add(col);
                    //Debug.Log("new shortest at x: " + col + " of val: " + shortestHeight);
                }
                else if (currHeight == shortestHeight) {
                    // add to random pool of equally good options
                    options.Add(col);
                    //Debug.Log("duplicate shortest at x: " + col + " of val: " + shortestHeight);
                }
            }
        ...
        private void EnemyDoAction() {
            ...
            if (null == _enemyPiece) {
                // find lowest line
                _enemyTargetCol = FindShortestCol(_enemyChunk);
                Debug.Log("target col: " + _enemyTargetCol);
                _enemyPiece = CreateAPiece((_minEx + _maxEx) / 2, _maxEy);
                ...
                if (!ValidPiece(tmpPiece, false)) {
                    ...
                    _enemyPiece = EnemyChooseAction(tmpPiece, AI_DEPTH);
